...
#define PLANE_CONST (2.0 * / LAMBDA * sin(REF_BEAM_ANGLE))
...
{
    return sin((distance(uv, point) % LAMBDA) * VAL_CONST);
}
...
{
    for(int x = threadId.x; x < ARRAY_WIDTH; x += blockDim.x)
    {
        for(int y = blockId.x; y < ARRAY_HEIGHT; y+= gridDim.x)
        {
            ...
            {
                pattern[index] += points[i].w * val(uv, points[i]) * intensity(uv, points[i]);
            }
        }
    }
...
int main(void)
{
    ...
    h_points[0] = {PATTERN_WIDTH / 2.0, PATTERN_HEIGHT / 2.0, 0.002, 1.0};
